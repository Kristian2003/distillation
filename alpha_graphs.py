...
ALPHA_PARAM_NAME = "alpha_kl_loss_weight"
DF_ALPHA_COL = "alpha_kl_loss_weight"

RESULTS_DIR = "./results_alpha"
OUTPUT_GRAPH_DIR = "./graphs_alpha"
...
def load_all_results(results_directory: str) -> pd.DataFrame:
    ...
    focusing on the alpha parameter.
    """
    all_data = []
    ...
    for file_path in json_files:
        try:
            ...
            alpha_value = args.get(ALPHA_PARAM_NAME)
            ...
            if alpha_value is not None:
                all_data.append({
                    DF_ALPHA_COL: float(alpha_value),
                ...
                print(f"Warning: '{ALPHA_PARAM_NAME}' not found in args for {file_path}")
    ...
    df = df.sort_values(by=DF_ALPHA_COL).reset_index(drop=True)
    return df

def plot_metrics(df: pd.DataFrame, output_dir: str):
    """
    Generates and saves plots based on the loaded data, with Alpha on the x-axis.
    """
    if df.empty:
    ...
    unique_alphas = sorted(df[DF_ALPHA_COL].unique())
    ...
    def _create_plot(y_gt_col, y_teacher_col, title, ylabel, filename_suffix, single_line_col=None, single_line_label=None, single_line_color='green'):
        ...
        if y_gt_col and y_gt_col in df.columns and not df[y_gt_col].isnull().all():
            plt.plot(df[DF_ALPHA_COL], df[y_gt_col], marker='o', linestyle='-', label="Student vs. Ground Truth (GT)")
            plotted_something = True
        if y_teacher_col and y_teacher_col in df.columns and not df[y_teacher_col].isnull().all():
            plt.plot(df[DF_ALPHA_COL], df[y_teacher_col], marker='x', linestyle='--', label="Student vs. Teacher")
            plotted_something = True
        if single_line_col and single_line_col in df.columns and not df[single_line_col].isnull().all():
            plt.plot(df[DF_ALPHA_COL], df[single_line_col], marker='s', linestyle='-.', color=single_line_color, label=single_line_label or single_line_col)
            plotted_something = True

        plt.title(title)
        plt.xlabel("Alpha (KL Loss Weight)")
        plt.ylabel(ylabel)
        plt.xticks(unique_alphas)
        if plotted_something:
            plt.legend()
        plt.grid(True, linestyle='--', alpha=0.7)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"alpha_vs_{filename_suffix}.png"))
        plt.close()
    ...
    for item in metrics_to_plot:
        ...
        if col_gt in df.columns and not df[col_gt].isnull().all():
            ...
            plt.plot(df[DF_ALPHA_COL], gt_values, marker='o', linestyle='-', color=metric_color, label=f"{metric_label_base} (GT)")
        ...
        if col_teacher in df.columns and not df[col_teacher].isnull().all():
            ...
            plt.plot(df[DF_ALPHA_COL], teacher_values, marker='x', linestyle='--', color=metric_color, label=f"{metric_label_base} (Teacher)")

    plt.title("Effect of Alpha (KL Loss Weight) on Key Assertion Metrics")
    plt.xlabel("Alpha (KL Loss Weight)")
...
if __name__ == "__main__":
    if not os.path.exists(RESULTS_DIR):
        ...
        print(f"Results directory '{RESULTS_DIR}' did not exist and was created.")
        print(f"Please place your JSON result files in '{RESULTS_DIR}'.")
    ...
    if not results_df.empty:
        ...
        print(f"\nNo data loaded. Please ensure JSON files are present in '{RESULTS_DIR}' and are valid.")